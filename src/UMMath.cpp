...
namespace UMMath{
   ...
   double GetScale(const char *units){
      ...
      if( UMUtility::AreEquivStrings(units,second) ){
         scale = 1.0;
      }else if( UMUtility::AreEquivStrings(units,millisecond) ){
         scale = 1E-3;
      }else if( UMUtility::AreEquivStrings(units,microsecond) ){
         scale = 1E-6;
      }else if( UMUtility::AreEquivStrings(units,nanosecond) ){
         scale = 1E-9;
      }else if( UMUtility::AreEquivStrings(units,picosecond) ){
         scale = 1E-12;
      }else if( UMUtility::AreEquivStrings(units,notdef) ){
         scale = -1;
      }else{
         std::cout << "[UMDAQ::GetScale]: Invalid units!  Exiting..." << std::endl;
         exit(1);
      }
      return scale;
   }
   ...
   //______________________________________________________________________________
   int StoreData(int verbosity,int i,int NPTS,UMPulse *aPulse,double *X,double *Y,double *EY){
      ...
      if(v_current!=0){
         ...
         if(verbosity>=3){  
            std::cout << "[UMMath::StoreData]: Voltage at zero crossing is zero!" << std::endl;
      ...
      if(start < 0){
         ...
         if(verbosity>=3){ 
            std::cout << "[UMMath::StoreData]: Invalid start point!  Setting to index = " << i << std::endl; 
            std::cout << "                              start = " << start << std::endl;
            std::cout << "                              end   = " << end   << std::endl;
         }
      }else if(start==0){
         ...
         if(verbosity>=3){
            std::cout << "[UMMath::StoreData]: Starting at index = " << start << std::endl; 
            std::cout << "                              start = " << start << std::endl;
            std::cout << "                              end   = " << end   << std::endl;
         }
      }
      ...
      if(k!=NPTS){
         NPTSUseable = k; 
         if(verbosity>=3){
            std::cout << "[UMMath::StoreData]: WARNING!  Do not have the expected number of data points! " << std::endl;
   ...
   double GetTimeOfCrossing(int verbosity,int method,int NPTSUseable,double X[],double Y[],double EY[], 
      ...
      if(method==kMidpoint){
         ...
         if(t0<X[0] || t0>X[SIZE-1]){
            if(verbosity>=3){
               std::cout << "[UMMath::GetTimeOfCrossing]: ERROR!  t0 is out of bounds!  Using linear interpolation instead..." << std::endl;
               std::cout << "                              t_min = " << X[0] << "\t" << "t_max = " << X[SIZE-1] << "\t" << "t0 = " << t0 << std::endl;
            }
            t0 = LinearInterpolationForX(v0,t_current,v_current,t_next,v_next);
            if(verbosity>=3) std::cout << "[UMMath::GetTimeOfCrossing]: linear interpolation: t_current = " << t_current << "\t" 
      ...
      if(verbosity>=3){  
         if(t0<0){
            std::cout << "[UMMath::GetTimeOfCrossing]: BAD CROSSING TIME!" << std::endl;
   ...
   int CountZeroCrossings(int verbosity,int method,int NPTS,int step,
      ...
      if(verbosity>=3) std::cout << "[UMMath]: Counting zero crossings..." << std::endl;
      ...
      int i=0;  // index for UMPulse data 
      do{
         ...
         if(v_prod > target){
            ...
            if(t0<0 || delta_v>0.100){   // we shouldn't see a 100 mV jump during the zero crossing 
               if(verbosity>=4){
                  std::cout << "[UMMath::CountZeroCrossings]: bad crossing for Zc = " 
               ...
               if(verbosity>=4){ 
                  std::cout << "[UMMath::CountZeroCrossings]: zero crossing counter reset to: " << cntr << std::endl;
                  std::cout << "[UMMath::CountZeroCrossings]: moving to index:                " << i       << std::endl;
               }
               continue; 
            }
            // passed t0 check 
      ...
      if(verbosity>=3) std::cout << "[UMMath::CountZeroCrossings]: Done." << std::endl;
